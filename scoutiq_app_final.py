...
def badge_html(rec):
    ...
    icon = {'STRONG BUY':'🏆','BUY':'✅','MONITOR':'👀','PASS':'❌'}.get(rec,'')
    return f'<span class="{cls}">{icon} {rec}</span>'
...
with st.sidebar:
    ...
    page = option_menu(
        menu_title=None,
        options=["Home", "Benchmarks", "Scout Engine", "Player Profile", "Market Intelligence", "About"],
        icons=["house-fill", "trophy-fill", "binoculars-fill", "person-vcard-fill", "graph-up-arrow", "info-circle-fill"],
        default_index=0,
        styles={
            "container": {"padding": "0", "background-color": "transparent"},
            "icon": {"color": "#8B9DC3", "font-size": "16px"},
            "nav-link": {
...
# ════════════════ PLAYER PROFILE ════════════════
    ...
    col1, col2 = st.columns(2)
    if 'Gls_90' in row.index:
        player_vals = {f: float(row.get(f, 0.0)) for f in features}
    else:
        fin_df = salah_fin if bench_sel == 'Mo Salah' else kdb_fin
        fin_row = fin_df[fin_df['Player'] == p_sel]
        if len(fin_row) > 0:
            player_vals = {f: float(fin_row.iloc[0][f]) if f in fin_row.columns else 0.0 for f in features}
        else:
            player_vals = {f: 0.0 for f in features}
    with col1:
    ...
    with col2:
        ...
        fd = [{'Feature':f, 'Pct': min(player_vals[f]/bench_v[f]*100,150) if bench_v[f]>0 else 0} for f in features]
    ...
    strengths = [lbl for f,lbl in [('Gls_90','goal output'),('Ast_90','assist creation'),('Sh_90','shot volume'),('SoT_90','shooting accuracy'),('SoT_pct','shot efficiency'),('G_per_Sh','finishing quality')] if bench_v[f]>0 and player_vals.get(f,0)>=bench_v[f]*0.70]
...
# ════════════════ MARKET INTELLIGENCE ════════════════
    ...
    with tab3:
        ...
        fig_bub.add_annotation(x=1.5,y=0.72,text="🎯 IDEAL ZONE",showarrow=False,font=dict(size=12,color='#2A9D8F'),bgcolor='rgba(42,157,143,0.13)',bordercolor='#2A9D8F')
...
# ════════════════ ABOUT ════════════════
    ...
    st.markdown("<h1>ℹ️ ABOUT SCOUTIQ</h1>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("""<div class='player-card'>
            ...
            <p style='color:#8B9DC3;line-height:1.8;'>Liverpool bought Mo Salah for £36.9M in 2017. Roma bought him for €15M in 2016 — with identical statistics. ScoutIQ identifies players at the €15M moment, not the £36.9M moment. The model has identified transfer market savings of up to <b style='color:#F4A261;'>€14.2M (Salah targets) and €17.2M (KDB targets) per signing</b>.</p>
            <h3>🔬 Methodology</h3>
            <p style='color:#8B9DC3;line-height:1.8;'>Built on the CRISP-DM framework across 7 phases. ML pipeline: StandardScaler → PCA (80.0% variance explained) → K-Means K=4 → Hybrid Similarity Scoring → Financial Decision Framework. Data: FBref.com (Season 2023/24) + Transfermarkt valuations (Aug/Sep 2024).</p>
    ...
    with col2:
        st.markdown("""<div class='player-card' style='border-color:#F4A261;'>
            <div style='font-family:Bebas Neue;font-size:1.5rem;color:#F4A261;'>🏷️ PRICING</div><hr style='border-color:#1E2D40;'>
        ...
        <div class='player-card' style='margin-top:1rem;'>
            ...
            <div style='color:#8B9DC3;font-size:0.85rem;line-height:2;'> Abdelrahman M. Elhosary<br> USM · ABW508 Analytics Lab<br>  Dr. Khaw Khai Wah <br> 264 players · 3 leagues · 34 valued<br> PCA 80.0% · K-Means K=4 · Hybrid Sim<br> Max saving: €14.2M (Salah targets) and €17.2M (KDB targets) per signing<br> 7 CRISP-DM phases · 4 gaps solved</div>
        </div>""", unsafe_allow_html=True)
    st.divider()
